...
@theme inline {
  ...
  --color-color-sidebar-ring: var(----color-sidebar-ring);
  --color-color-sidebar-border: var(----color-sidebar-border);
  --color-color-sidebar-accent-foreground: var(----color-sidebar-accent-foreground);
  --color-color-sidebar-accent: var(----color-sidebar-accent);
  --color-color-sidebar-primary-foreground: var(----color-sidebar-primary-foreground);
  --color-color-sidebar-primary: var(----color-sidebar-primary);
  --color-color-sidebar-foreground: var(----color-sidebar-foreground);
  --color-color-sidebar: var(----color-sidebar);
  --color-color-chart-5: var(----color-chart-5);
  --color-color-chart-4: var(----color-chart-4);
  --color-color-chart-3: var(----color-chart-3);
  --color-color-chart-2: var(----color-chart-2);
  --color-color-chart-1: var(----color-chart-1);
  --color-color-ring: var(----color-ring);
  --color-color-input: var(----color-input);
  --color-color-border: var(----color-border);
  --color-color-destructive-foreground: var(----color-destructive-foreground);
  --color-color-destructive: var(----color-destructive);
  --color-color-accent-foreground: var(----color-accent-foreground);
  --color-color-accent: var(----color-accent);
  --color-color-muted-foreground: var(----color-muted-foreground);
  --color-color-muted: var(----color-muted);
  --color-color-secondary-foreground: var(----color-secondary-foreground);
  --color-color-secondary: var(----color-secondary);
  --color-color-primary-foreground: var(----color-primary-foreground);
  --color-color-primary: var(----color-primary);
  --color-color-popover-foreground: var(----color-popover-foreground);
  --color-color-popover: var(----color-popover);
  --color-color-card-foreground: var(----color-card-foreground);
  --color-color-card: var(----color-card);
  --color-color-foreground: var(----color-foreground);
...
## Feature: 4 AI Agents (BYOK) — verbatim user request

"i have create a youtube kid generation here so In this I want you to help me create 3 other agents one for script writing, the other for video generating, and the next one is for self monitoring on both ends if there's any mismatch. and also everything should be in one single page no more route. In those 3 agents when user clicked on it user can pick any models as long as they match them selve, text to video, text to text, text to speech etc... wait so create 4 agents because, scripts, video and text to speech. and user can add the base url or endpoints of it and api key then suer can click save and fecth mdoel"

Later addition: "when I asked them to generate an AI movie, it will send to me so then I can tell it to post on my youtube channel"

### Structured requirements (implement alongside the chat input component)
1. Single page, NO routing — agents live as cards/panels beside the "Ask anything" chat bar.
2. Four agents:
   - Script Writer — capability: text-to-text
   - Video Generator — capability: text/image-to-video (falls back to keyframe image gen)
   - Voice (TTS) — capability: text-to-speech
   - Monitor — self-checks the other three agents (config + live probe) and flags any mismatch between expected capability and the endpoint's actual response.
3. BYOK per agent: user supplies base URL/endpoint + API key, picks any model as long as it matches the agent's capability type, clicks "Save + Fetch models" (persist config, GET /v1/models or provider equivalent, filter list by capability).
4. Ask-anything flow: movie request → script → frames/animation → TTS → assembled video → deliverable shown to the user → user can then tell the app to post it to their YouTube channel (posting stays user-confirmed).
5. Keys never leave the browser except to the user-configured endpoint; never log them.
